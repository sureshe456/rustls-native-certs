...
reset() {
  ...
  sudo security remove-trusted-cert -d /Library/Keychains/System.keychain -c $ANY_CA_PEM
  #sudo security remove-trusted-cert -d integration-tests/one-existing-ca.pem || true
  list | grep "$ANY_CA_SUBJECT"
}
